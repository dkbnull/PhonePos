<?php
	...
	// 加载日结信息
	if(strcmp($method,"loadMonthover")==0){
	...
	// 日结
	else if (strcmp($method,"monthover")==0){
		$data=$request["data"];
		$date=$request["date"];
		$total=$request["total"];
		$username=$request["username"];
		...
		// 是否已经日结
		$sql="select * from monthover "
			."where parcode='".$parcode."' "
			."and date='".$date."'";
		$result=$conn->query($sql);
		
		if ($record=$result->fetch_assoc()) {
			echo '{"msgcode":"2","msgmain":"has monthover"}';
			return false;
		}

		$sql="insert into monthover (parcode, date, operator, total, info) "
			."values ('"
			.$parcode."', '"
			.$date."', '"
			.$username."', '"
